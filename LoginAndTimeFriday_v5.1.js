...
var iframe = document.getElementById('app');
	...
	var table = innerDoc.getElementById("weekmodtbl");
	if (table == null) {
	...
	var colInText = col.textContent || col.innerText;	/* chrome uses innerText, firefox does not know it.
			firefox adds some line breaks.*/
	colInText = colInText.replace(/(\r\n|\n|\r)/gm,"");
	var weeksTime = Math.floor(Number(colInText)*60);
	
	var goal = linearInTime + lunchPeriod + ((38.5*60) - weeksTime);
	var goalHours = parseInt(goal/60, 10);
	var goalMins = goal - (goalHours * 60);
	if (goalMins < 10) {goalMins = "0"+goalMins;};	
	...
	/* calc effective weeksTime until now */
	var weeksTimeHours = parseInt(weeksTime/60, 10);
	var weeksTimeMinutes = weeksTime - (weeksTimeHours * 60);
	var effectiveWeeksHours = workedHours + weeksTimeHours;
	var effectiveWeeksMinutes = workedMinutes + weeksTimeMinutes;

	if (effectiveWeeksMinutes >= 60) { 
		effectiveWeeksMinutes =  effectiveWeeksMinutes - ((Math.floor(effectiveWeeksMinutes / 60))*60); 
		effectiveWeeksHours += 1;
	}
	...
	if (effectiveWeeksMinutes < 10) {
		effectiveWeeksMinutes = "0"+effectiveWeeksMinutes;
	}	
	sStringz = (sStringz) + "\neffective weeks time: " + (effectiveWeeksHours) + ":" + (effectiveWeeksMinutes);
	paintCurrentTimeInTable(table, my1date);
...
function paintCurrentTimeInTable(table){
	...
	/* find out cell and paint it */
	/* row of current day  (see function isFriday() ) */
	/*  add constant 2 for table (table row #2 is Monday) */
	var rowIdx = indexOfDay + 2; 
...
function clickLogin() {
	var iframe = document.getElementById('app');
	var innerDoc = iframe.contentDocument || iframe.contentWindow.document;
	var elm = innerDoc.getElementById('menutext');
	elm.focus();
	simulateClick(elm);
	return;
}
 ...
 */		
			...
			colInText = colInText.replace(/(\r\n|\n|\r)/gm,"");
			/* chrome uses innerText, firefox does not know it.
					firefox adds some line breaks.*/
			
			/*get kommen*/
			if(col.id == "K3" && colInText == "K" && !kommenFound) {
			
				startCounting = true;
				kommenFound = true;
			}
			/*get GM*/
			if(col.id == "K3" && colInText == "GM") {
				startCounting = true;
				/* GM found before, use first occurrence --> reset startCounting */
				if (GMfound){
					startCounting = false;
				}
				GMfound = true;
			...
			/*get KM*/
			/*if(col.id == "K3" && (colInText == "KM" || colInText == "G" )) {*/
			if(col.id == "K3" && (colInText == "KM")) {
				startCounting = true;
				KMfound = true;
			}
			if(startCounting){
			...
			/*get time*/
			if (startCounting && col.id == "T5" ) {
				var buchung = row.cells[j-cnt+1].textContent || row.cells[j-cnt+1].innerText;
				buchung = buchung.replace(/(\r\n|\n|\r)/gm,"");
				times[buchung] = colInText.split(":");
				startCounting = false;
				cnt = 0;
	...
	var x5Date = innerDoc.getElementsByName("x5");
	if (x5Date == null) {
		alert('please login to ePZE -> Buchen -> today');
		return;
	}
...
function isFriday() {	 
	...
	myDate.setDate(dateText[2]);
	/* find out the day index: [0-6] begins sunday */
	indexOfDay = myDate.getDay();
	/* friday == 5 */
...
function calc(){	
	...
	if (GMfound) {
		...
		var lunchOutMinutes = parseInt(times["GM"][1], 10);
/*	} else if (KMfound) { */
		/* if no GM then booked out right before lunch, use KM (or G) */
/*		var lunchOutHour = parseInt(times["KM"][0], 10);
		var lunchOutMinutes = parseInt(times["KM"][1], 10);
*/	
	} else {
		var lunchOutHour = 0;
	...
	if (KMfound) {
		var lunchInHour = parseInt(times["KM"][0], 10);
	...
	/* 	normal day: 7h + 50min
		normal friday: 7h 20min
		
		shortFR weekDay: 8h + 30min
		shortFR friday; 4h + 30min
		
		adjusted for week: 38.50h - Sum all worked 
		*/
	var outHour = 7;
