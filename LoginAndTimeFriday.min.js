javascript:var persNumber='7846';var persPass='KEbuchen';var times=new Array();var kommenFound=false;var GMfound=false;var KMfound=false;var linearInTime;var lunchPeriod;var inHour;var inMinutes;var timeWorked;var timeWorked_ShortFR;var dateText;var misids;var currentLinearTime;var indexOfDay;function doIt(){login();setTimeout('getTodaysTimes()',1000);setTimeout('clickWoche()',1500);setTimeout('addWoche()',3000);}
...
var my1date=new Date();var current_hourInMins=my1date.getHours()*60;var current_minute=my1date.getMinutes();currentLinearTime=current_hourInMins+current_minute;var effectiveLinearTime=currentLinearTime-linearInTime-lunchPeriod;var workedHours=parseInt(effectiveLinearTime/60,10);var workedMinutes=effectiveLinearTime-workedHours*60;if(workedMinutes<10){workedMinutes="0"+(workedMinutes);};sStringz=(sStringz)+"\n\neffective work time until now: "+(workedHours)+":"+(workedMinutes);paintCurrentTimeInTable(innerDoc.getElementById("table1"),my1date);alert(sStringz);}
